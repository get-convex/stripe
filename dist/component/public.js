...
export const getPayment = query({
    ...
    handler: async (ctx, args) => {
        const payment = await ctx.db
            .query("payments")
            .withIndex("by_stripe_payment_intent_id", (q) => q.eq("stripePaymentIntentId", args.stripePaymentIntentId))
            .unique();
        if (!payment)
            return null;
...
export const updateSubscriptionMetadata = mutation({
    ...
    handler: async (ctx, args) => {
        const subscription = await ctx.db
            ...
            .unique();
        if (!subscription) {
            throw new Error(`Subscription ${args.stripeSubscriptionId} not found in database`);
        }
